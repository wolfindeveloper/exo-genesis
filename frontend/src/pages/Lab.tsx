        ...
        {slots.map((slot, i) => (
          <motion.div
            ...
            className={`aspect-square rounded-xl border-2 flex items-center justify-center text-center p-2 cursor-pointer transition-colors relative overflow-hidden ${
              slot
                ? `${rarityColors[elementData[slot.item_config_id]?.rarity] || 'border-slate-500/30'} border-solid bg-space-600/80`
          ...
          >
            {slot ? (
              ...
              >
                <div className="text-2xl mb-1">{elementData[slot.item_config_id]?.emoji || '?'}</div>
                <div className="text-[10px] leading-tight text-slate-300">{elementData[slot.item_config_id]?.name || slot.item_config_id}</div>
              </motion.div>
            ) : (
            ...
            {Array.from({ length: 12 }).map((_, i) => (
              <motion.div
                ...
                style={{
                  ...
                  boxShadow: i % 2 === 0 ? '0 0 6px #22d3ee' : '0 0 6px #a855f7',
                }}
                variants={{
              ...
              <div>
                ...
                {lastExperiment.is_first_discoverer && (
                  <motion.p
                    ...
                    transition={{ duration: 0.5 }}
                    className="text-xs text-neon-amber font-display uppercase tracking-wider"
                  >
                    🏆 Первооткрыватель!
                  </motion.p>
                )}
          ...
          {elements.map((item) => {
            ...
            return (
              <motion.button
                ...
                } bg-space-700/50 hover:bg-space-600/80`}
              >
