...
def _make_artifact(combo: list[str], elements: list[dict], rng: random.Random) -> dict:
    ...
    return {
        ...
        "artifact_name_key": f"{artifact_id}_name",
        "artifact_desc_key": f"{artifact_id}_desc",
        "tier": result_tier,
    ...
    for i in range(len(eids)):
        for j in range(i + 1, len(eids)):
            if rng.random() < 0.3:
                ...
                recipes[key] = _make_artifact([eids[i], eids[j]], elements, rng)
        ...
        for j in range(i + 1, len(eids)):
            for k in range(j + 1, len(eids)):
                if rng.random() < 0.15:
                    ...
                    recipes[key] = _make_artifact([eids[i], eids[j], eids[k]], elements, rng)
