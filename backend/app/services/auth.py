...
def _validate_init_data(init_data_raw: str) -> dict:
    ...
    # Check_string INCLUDES signature (only hash is excluded)
    if received_hash:
        hmac_data = parsed.copy()
        if received_signature:
            hmac_data["signature"] = received_signature
        check_string = "\n".join(
            f"{k}={v}" for k, v in sorted(hmac_data.items())
        )
        secret_key = hmac.new(
            b"WebAppData",
    ...
    # Check_string excludes BOTH hash and signature
    if received_signature:
